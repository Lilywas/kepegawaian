...
class UserModel extends CI_Model
{
	...
	function get_detail($id_pegawai)
	{
		$this->db->select('*');
	...
	function get_status($id_pegawai)
	{
	...
	function delete_komp($id_komp)
	{
		$this->db->where('id_kompetensi', $id_komp);
	...
	function get_all_riwayat($id_riwayat)
	{
		$this->db->select('*');
		$this->db->from('riwayat_unit_kerja');
		$this->db->where('id_riwayat_unit_kerja', $id_riwayat);
	...
	function get_sub_unit_kerja($id_sub)
	{
		$hasil = $this->db->query("SELECT * FROM sub_unit_kerja WHERE id_unit='$id_sub'");
		return $hasil->result();
	}
	function get_sub_unit($id_sub)
	{
		$hasil = $this->db->query("SELECT sub_pegawai FROM pns_local WHERE id_pegawai='$id_sub'");
		return $hasil->result();
	}
	function get_sub_unit_fromsub($id_sub)
	{
		$this->db->select('id_unit');
		$this->db->from('sub_unit_kerja');
		$this->db->where('id_sub_unit', $id_sub);

		return $this->db->get();
	}
	function get_sub_unit_fromunit($id_sub)
	{
		$this->db->select('*');
		$this->db->from('sub_unit_kerja');
		$this->db->where('id_unit', $id_sub);
	...
	function delete_sub_by_unit($id_unit)
	{
		$this->db->where('id_unit', $id_unit);
